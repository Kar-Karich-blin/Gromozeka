...
"2) конец связи - окончить разговор \n";
    ...
    while (true) {
        while(key == 2) {
            ...
            // Запись в .txt
            std::ofstream outfile("chat2.txt", std::ios_base::app); 
            if (outfile.is_open()) {

                if(message == "приём"){ key = 1; }
                if(message == "конец связи"){ key = 0; }

                message = "Клиент 2: " + message;
                kode(sif, message);
            ...
            if (infile.is_open()) {
                while (std::getline(infile, message)) {
                    if(!message.empty()){
                        ...
                        if(message == "Клиент 1: приём"){ key = 2; }
                        if(message == "Клиент 1: конец связи"){ key = 0; }
                ...
                if (notEmpty) {
                    std::ofstream outfile("chat2.txt", std::ios::trunc);
                    outfile.close();
                }
            }
        }
